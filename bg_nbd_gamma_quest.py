...
df["order_num_sum"] = df["order_num_total_ever_online"] + df["order_num_total_ever_offline"]
df["customer_value_sum"] = df["customer_value_total_ever_online"] + df["customer_value_total_ever_offline"]

df = df.apply((lambda columns: pd.to_datetime(columns) if "date" in columns.name else columns))
...
cltv_df = pd.DataFrame({"customer_id": df["master_id"],
                       ...
                       astype("timedelta64[D]") / 7,
                        ...
                        "monetary_cltv_avg": df["customer_value_sum"] / df["order_num_sum"]})
