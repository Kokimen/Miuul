...
# hayat kurtaran!: [] içerisinde verilen değişkenler, dataframe içerisinden seçilir.
df[num_cols].head()

# hayat kurtaran!: agg fonksiyonu bir df'ye uygulanırsa ve içerisine sözlük gönderilirse, gönderilen sözlüğün içerisindeki
# değişkenler df'de mevcut ise bu değişkenleri key bölümünde tutar. Value bölümüne girilen bütün değişkenleri de key'lerin
# yanına getirir ve otomatik uygular.
df[num_cols].agg(new_dict)

###PYTHON VERİ ANALİZİ###
# Numpy: matematiksel işlemler
# Pandas: veri denilince akla gelen
# Veri Görselleştirme: Matplotlib (low) & Seaborn (high)
# Gelişmiş Fonksiyonel Keşifçi Veri Analizi: Büyük ölçekli projelerin fonksiyonel seviyede gerçekleştirilmesi
...
# 10 adet sıfırdan oluşan int liste
np.zeros(10, dtype = int)

# 0-10 arasında, 10 random int liste
np.random.randint(0, 10, size = 10)

# ortalaması 10, standart sapması 4 olan, 3x4 lük sayı litesi
...
a[0, 0]  # sol taraf satır, sağ taraf sütun
a[2, 3] = 2.9  # float değer olmaz, 2 değeri girilir, numpy tek tip veri tutar
...
# linalg.solve: bilinmeyen değerleri hesaplar
np.linalg.solve(a, b)

######PYTHON PANDAS KÜTÜPHANESİ VERİ ANALİZİ########
# VERİ OKUMA#
import pandas as pd
...
df.columns
df.index
# betimsel özet bilgi, T transpose
df.describe().T
# değişkenlerden biri null mu
df.isnull().values.any()
# değişkenlerde eksik bilgi var mı
df.isnull().sum()
df["sex"].head()
# hangi değişkende kaç sınıf var
df["sex"].value_counts()
...
# belirli bir aralıkta seçim
df[0:13]
# drop veri siler
df.drop(0, axis = 0).head()
delete_indexes = [1, 3, 5, 7]
df.drop(delete_indexes, axis = 0).head(6)
# işlemi kalıcı yapmak inplace
...
# birden fazla değişkeni seçmek
df[['age', 'alive']]
col_names = ['age', 'adult_male', 'alive']
df[col_names]

df['age'] = df['age'] ** 2
df['age3'] = df['age'] / df['age']
df.drop('age3', axis = 1).head()
# axis = 1 sütun sil demek
df.drop(col_names, axis = 1).head()

# LOC & ILOC
# contains: stringleri ara, tilda işareti: olmayanları anlamına gelir. loc df'lerde seçme işlemleri için kullanılan özel yapı.
df.loc[:, ~df.columns.str.contains('age')].head()

# iloc: integer based, yani index bilgisi vererek seçim yapar
# loc: label based yani indexlerde ki labellara göre seçim yapar

# integer based seçim
df.iloc[0:3]  # 3'e kadar seçer.

# label based seçim
df.loc[0:3]  # 3 dahil seçer.
